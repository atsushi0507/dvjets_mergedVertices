...
nominalFile = r.TFile("rootfiles/sigAndMass_{}.root".format(dataSet), "READ")
tcFile = r.TFile("rootfiles/sigAndMass_{}_trackCleaning.root".format(dataSet), "READ")
...
for ntrk in ntrkList:
    noDVSelDict[ntrk] = nominalFile.Get("mergedMass_{}".format(ntrk))
    DVSelDict[ntrk] = nominalFile.Get("mergedMass_DVsel_{}".format(ntrk))

    trackCleaningDict[ntrk] = tcFile.Get("mergedMass_{}".format(ntrk))
    fullSelDict[ntrk] = tcFile.Get("mergedMass_DVsel_{}".format(ntrk))
    ...
    sampleLabel.DrawLatex(0.56, 0.73, "Significance method, " + getNtrkLabel(ntrk))
    leg.Draw()
