...
def decorateLegend(leg):
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.SetTextFont(42)
    leg.SetTextSize(0.03)

def saveFig(c, directory, name):
    c.SaveAs("{}/{}.pdf".format(directory, name))
...
def setColor(hist, col=r.kBlack):
    hist.SetLineColor(col)
    hist.SetMarkerColor(col)

def drawLabels(leg, label):
    leg.Draw()
    ATLASLabel(0.150, 0.95, label)
