...
parser.add_argument("-tc", "--doTrackCleaning", action="store_true", help="Do track cleaning")
...
parser.add_argument("-n", "--nEvents", default=-1, help="Set maximum events to process")
args = parser.parse_args()

outputTag = "mc16{}".format(args.tag)
SR = args.SR
doTC = args.doTrackCleaning
...
fileOut=ROOT.TFile("outputFiles/mergedVerticesTreeMC_%s_trackCleaning_%s.root"%(outputTag, SR),"RECREATE")
...
for event in treeIn:
    ...
    if (SR == "highPtSR"):
        if not (ord(event.DRAW_pass_triggerFlags) and ord(event.DRAW_pass_DVJETS) and ord(event.BaselineSel_HighPtSR)):
            continue
    if (SR == "tracklessSR"):
        if not (ord(event.DRAW_pass_triggerFlags) and ord(event.DRAW_pass_DVJETS) and ord(event.BaselineSel_TracklessSR)):
            continue
