...
def trackCleaning(tree, idv):
    ...
    for itrack in range(len(tree.dvtrack_ptWrtDV)):
        ...
        if ( (tree.DV_rxy[idv] > 119.3) and (d0sig < 10.) and tree.dvtrack_isAssociated[itrack] == 0):
            continue
        if (tree.DV_rxy[idv] < 25.): # inside BP
            if (tree.dvtrack_isAssociated[itrack]):
                continue
...
def getEvents(tree, eventID, doUnblind):
    ...
    for entry in range(tree.GetEntries()):
        ...
        pileup = tree.actualInteractionsPerCrossing
        PV = [tree.PV_x, tree.PV_y, tree.PV_z]
        ...
        if (len(vertex) == 0):
            continue
        events.append([eventID, vertex, tracks, covariance, jets, pileup, PV])
    return events

def getEvents_inMaterial(tree, eventID, doUnblind):
    events = []
    pileup = 0
    for entry in range(tree.GetEntries()):
        ientry = tree.LoadTree(entry)
        ...
        for idv in range(tree.DV_n):
            ...
            if (not tree.DV_passChiSqCut[idv]):
                continue
            if (tree.DV_passMaterialVeto[idv]):
                continue
            if (not doUnblind):
                if (tree.DV_m[idv] > 10. and tree.DV_nTracks[idv] >= 5):
                    continue
...
def getRegion(rxy):
    ...
    if (rxy >= 25. and rxy < 38.):
        region = 1
    if (rxy >= 38. and rxy < 120.):
        region = 2
    if (rxy >= 120. and rxy < 300.):
        region = 3
    return region
...
def isMerged(track1, track2, track3, track4, charge1, charge2, charge3, charge4):
    isMerged = False
    
    m_12 = (track1 + track2).M()
    m_13 = (track1 + track3).M()
    m_14 = (track1 + track4).M()
    m_23 = (track2 + track3).M()
    m_24 = (track2 + track4).M()
    m_34 = (track3 + track4).M()
    
    isKshort_12 = True if (r.TMath.Abs(m_12 < m_kshort) < 0.05 and charge1*charge2 == -1) else False
    isKshort_13 = True if (r.TMath.Abs(m_13 < m_kshort) < 0.05 and charge1*charge3 == -1) else False
    isKshort_14 = True if (r.TMath.Abs(m_14 < m_kshort) < 0.05 and charge1*charge4 == -1) else False
    isKshort_23 = True if (r.TMath.Abs(m_23 < m_kshort) < 0.05 and charge2*charge3 == -1) else False
    isKshort_24 = True if (r.TMath.Abs(m_24 < m_kshort) < 0.05 and charge2*charge4 == -1) else False
    isKshort_34 = True if (r.TMath.Abs(m_34 < m_kshort) < 0.05 and charge2*charge4 == -1) else False

    category = 0 
    if ((isKshort_12 and isKshort_34) or (isKshort_13 and isKshort_24) or (isKshort_14 and isKshort_23)):
        isMerged = True
        ...
        if (isKshort_14 and isKshort_23):
            category = 3
        if ((isKshort_12 and isKshort_34) and (isKshort_13 and isKshort_24)):
            category = 4
        if ((isKshort_12 and isKshort_34) and (isKshort_14 and isKshort_23)):
            category = 5
        if ((isKshort_13 and isKshort_24) and (isKshort_14 and isKshort_23)):
            category = 6
        if ((isKshort_12 and isKshort_34) and (isKshort_13 and isKshort_24) and (isKshort_14 and isKshort_23)):
            category = 7

    return isMerged, category
...
def isSideband(mdv, mean, sigma):
    ...
    isSideband = True if (isLower or isHigher) else False
